...
<form action="<?php echo htmlspecialchars($ac_apply_url, ENT_QUOTES, 'UTF-8');?>" method="post" enctype="multipart/form-data" onSubmit="return disabledSubmit();">
	...
	<input name="siteminorversion" value="<?php echo htmlspecialchars($site_minor_version, ENT_QUOTES, 'UTF-8');?>" type="hidden" />
...
?>
<script src="<?php echo htmlspecialchars($openPlatformUrl, ENT_QUOTES, 'UTF-8');?>sitepush.php?a=platformPush" charset="UTF-8"></script>
...
?>
		<tr>
			...
			<td><span style="line-height:25px;"><?php echo htmlspecialchars($info[v], ENT_QUOTES, 'UTF-8');?></span></td>
			<td><div class="fun_tips"></div></td>
		</tr>
		<?php  } ?>
	</table>
	<div class="tac p10">
		<form name="reset" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=appcenter&c=server'; ?>" method="post" style="display:inline;">
			<button type="submit" class="btn btn_big btn_submit">继续</button>
		<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
		<form name="reset" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?operate=reset&m=appcenter&c=server&a=check'; ?>" method="post" style="display:inline;">
...
<div id="error_tips">
	...
	<div class="error_cont">
		<ul>
			...
			<li>请尝试：</li>
			<li>1、<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?operate=check&m=appcenter&c=server'; ?>">[环境监测]</a>，查看问题所在。待各指标结果正常后，再重试。</li>
			<li>2、若仍无法连接成功，请在<a href="http://www.phpwind.net/thread-htm-fid-131.html" target="_blank">[这里]</a>反馈给官方人员，我们竭诚为您解决。</li>
		</ul>
		<div class="error_return"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=appcenter&c=server'; ?>" class="btn">重试</a></div>
	</div>
</div>
...
<div id="error_tips">
	...
	<div class="error_cont">
		...
		<div class="error_return"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=appcenter&c=server'; ?>" class="btn">刷新</a></div>
	</div>
</div>
<?php  } ?>
</div>
